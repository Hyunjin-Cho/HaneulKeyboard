...
- [ ] 🚨 **템플릿 선택 화면이 아니라 바로 작성 화면**이 열린다. 2026-09-21에 저장소에 첫 이슈 템플릿(`.github/ISSUE_TEMPLATE/word-suggestion.md`)이 생겼는데, 템플릿이 있는 저장소에서 `/issues/new?title=…`이 **템플릿 고르기(chooser) 화면에 가로채이지 않는지는 이 세션에서 확인하지 못했다**(로그인 없이는 응답을 볼 수 없어 curl로 판별 불가). 고르기 화면이 뜨면 미리 채운 내용이 날아가므로, 그때는 **그 템플릿 파일을 지우면** 해결된다(앱 URL은 템플릿과 무관하다). 결과를 #55에 기록할 것.
