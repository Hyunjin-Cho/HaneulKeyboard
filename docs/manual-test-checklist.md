# HaneulKeyboard 수동 테스트 체크리스트

> **기준일:** 2026-09-21 · "(프로브가 확인 — scripts/postinstall_probe.sh N번)"이 붙은 줄은 `bash scripts/postinstall_probe.sh`의 N번 항목이 기계로 대신 확인한다(#52). 표기 없는 줄은 사람이 본다.

대상: **빌드 41 이상**의 서명·설치 완료된 배포본. 테스트 전에 앱과 `HaneulKeyboardIM.app` 빌드 번호가 대상 빌드와 일치하는지 확인한다. (프로브가 확인 — scripts/postinstall_probe.sh 1번)

## 메뉴바와 설정

- [ ] 앱 실행 후 메뉴바 **오른쪽** 상태 영역에 HaneulKeyboard `한` 또는 `A` 아이콘이 나타난다.
- [ ] 아이콘을 클릭하면 메뉴가 아이콘 아래의 정상 위치에 열린다(화면 왼쪽 위 등 엉뚱한 위치에 열리지 않는다).
- [ ] `설정...`을 누르면 설정 창이 열리고 조작 가능하다.
- [ ] 설정 창을 닫은 뒤 다시 `설정...`을 누르면 창이 정상적으로 재열린다.

### 탭 5개 (#60, 2026-09-21)

- [ ] 설정 창이 탭 **5개**(일반 · 영타 변환 · 개인 사전 · 업데이트 · 고급)로 열리고, 처음 열면 「일반」 탭이 보인다.
- [ ] 탭 5개를 차례로 눌러 전환되고, **이름과 아이콘이 빈칸 없이** 보인다(없는 SF Symbol은 조용히 빈칸으로 렌더된다).
- [ ] 창 크기는 **640×660 고정**이고 크기 조절이 안 되며, 탭을 바꿔도 창이 튀지 않는다.
- [ ] 기본 상태(목록이 비었거나 몇 줄)에서 **어느 탭에도 세로 스크롤바가 없다.**
- [ ] 「일반」: 상태 카드가 **두 줄**(설치 배지 / 현재 입력 모드)이고, 설치/미설치 어느 쪽이어도 높이가 같다. `입력 소스 설정 열기`가 시스템 설정을 연다.
- [ ] 「영타 변환」: 영타 자동 변환 토글 · 되돌리기 키 팝업 · `바뀌지 않은 단어도 되돌리기` 토글 · 앱별 끄기 목록이 한 탭에 모여 있고 전부 즉시 반영된다.
- [ ] 되돌리기 키 팝업에 마우스를 올리면 툴팁 "고른 조합이 다른 앱의 단축키와 겹치면 다른 조합을 고르세요."가 뜬다(캡션에서 툴팁으로 옮긴 문구).
- [ ] 「개인 사전」: 세그먼트 `변환 추가 | 변환 금지 | 최근 되돌림`을 좌우로 눌러도 **목록 아래 경계가 눈에 띄게 움직이지 않는다**(설계 허용 오차 ±1행, 실측 4pt).
- [ ] 「개인 사전」 → `단어 제안하기...` → 시트(480×420)가 뜬다. 값을 채우면 `GitHub에서 열기`가 활성화되고, 누르면 브라우저가 열린 뒤 **시트가 닫힌다**. `취소`·Esc로도 닫힌다.
- [ ] 「영타 변환」 → `실행 중인 앱에서 추가...` 시트(440×380)가 뜨고, 추가하면 뒤 목록에 즉시 반영되며 `닫기`·Esc로 닫힌다.
- [ ] 「업데이트」: 상태줄이 버튼 행 **위**에 있고, `업데이트` 버튼은 새 버전이 있을 때만 나타난다.
- [ ] 「고급」: `전체 제거...`만 단독으로 있고, 누르면 "하늘키보드를 모두 지울까요?" 확인 창이 뜬다(`모두 지우기` / `취소`).
- [ ] 시스템 설정에서 강조 색을 파랑이 아닌 색(예: 보라)으로 바꾸면 「업데이트」 탭의 "새 버전 …" 줄 색이 **따라 바뀐다**(`.tint`).
- [ ] 다크 모드로 바꾼 뒤 5개 탭을 모두 보고, 빈 상태 문구("추가한 단어가 없습니다." · "끈 앱이 없습니다." · "되돌린 변환이 없습니다.")가 **읽힌다**.
- [ ] 목록을 가득 채운 상태(개인 사전 20개, 앱별 끄기 10개)에서도 창 높이가 변하지 않고 목록만 안에서 스크롤된다.

## 온보딩

- [ ] 첫 실행 시 `시작하기...` 온보딩을 열고 모든 단계를 완료할 수 있다.
- [ ] 온보딩 완료 후 메뉴에 `시작하기 다시 보기...`가 나타난다.
- [ ] `시작하기 다시 보기...`로 온보딩을 재열고 단계 이동·닫기가 정상 동작한다.

## 입력 소스와 전환 키

- [ ] macOS 입력 메뉴에서 하늘키보드를 선택하면 메뉴바 표시가 `한`으로 갱신된다.
- [ ] ABC 입력 소스로 바꾸면 메뉴바 표시가 `A`로 갱신된다.
- [ ] 입력 소스를 여러 번 바꿔도 `한`/`A` 표시가 누락·지연 없이 현재 상태와 일치한다.
- [ ] 하늘키보드 활성 상태에서 Caps Lock을 짧게 누르면 한↔영 입력 소스가 전환되고 이어서 입력한 문자가 올바른다.
- [ ] Caps Lock을 길게 누르는 macOS 대문자 Caps Lock 동작과 짧게 누르는 한영 전환이 구분된다.
- [ ] **Caps Lock 시스템 옵션 OFF** (#56, 2026-09-21): 시스템 설정 → 키보드 → 입력 소스의 "Caps Lock으로 ABC 전환" 옵션을 **꺼둔** 상태에서 Caps Lock을 눌러도 언어가 바뀌지 않는다(바뀌면 OS 버그 — 우리는 `TICapsLockLanguageSwitchCapable`로 OS에 위임하므로 우리 코드로 못 고친다. Apple Community 2025-09 리포트: 옵션이 꺼져 있어도 Caps Lock이 언어를 바꿈). 확인 후 옵션을 다시 켠다.
- [ ] 영타 자동 변환 대상 단어를 확정한 뒤 Shift+Space를 누르면 영타↔한글 결과가 되돌려지고, 다시 누르면 역방향으로 토글된다.
- [ ] 토글 대상이 아닌 문장에서 Shift+Space가 인접 텍스트를 훼손하지 않는다.
- [ ] **ABC 없음** (#45, 2026-09-20): 입력 소스에서 ABC를 빼고 U.S.(또는 British·Dvorak) 같은 다른 영문 자판만 둔 상태에서 메뉴 "영어로 전환"을 누르면 **그 자판으로** 전환되고 메뉴바가 `A`가 된다.
- [ ] **영문 자판 없음**: 영문 자판이 하나도 켜져 있지 않을 때 "영어로 전환"을 누르면 "영어로 전환하지 못했어요" 알림이 뜬다(조용히 실패하지 않는다). 하늘키보드를 입력 소스에서 뺀 상태에서 "한국어로 전환"을 누르면 "한국어로 전환하지 못했어요" 알림이 뜬다.

### 터미널 — Shift+Space 되돌리기 미지원 (#30)

터미널에 입력된 글자는 즉시 셸 프로세스 소유가 되어 앱조차 IME 요청으로 회수할 수단이 없다. 따라서 되돌리기는 **원리적으로 불가능**하며, 이 항목들은 "되게 만드는 것"이 아니라 **실패가 조용하고 일관되게 끝나는지**를 확인한다. (2026-09-21, #54: 되돌리기 키를 다른 조합으로 바꿔도 같다 — 키가 아니라 클라이언트의 문제.)

- [ ] Terminal.app에서 영타 변환 후 Shift+Space를 여러 번 눌러도 텍스트가 훼손되지 않는다(변화 없음이 정상).
- [ ] Ghostty에서도 같은 동작 — 되돌려지지 않고, **스페이스가 대신 끼어들지도 않는다**(과거엔 스페이스만 늘어났다).
- [ ] 두 터미널의 동작이 서로 같다.
- [ ] 터미널에서 영타 자동 변환 자체는 정상 동작한다(되돌리기만 불가).
- [ ] 터미널에서 오변환이 나면 백스페이스로 지우고 다시 칠 때 재변환되지 않는다(수동 복구 경로).
- [ ] (#54) 설정 → 「영타 변환」 탭 「되돌리기」 절 아래에 터미널에서는 되돌리기가 안 된다는 안내 문구가 보이고, README "알려진 문제"에도 같은 항목이 있다.

### 되돌리기 키 선택 · 앱별 자동 변환 끄기 (#54, 2026-09-21)

자동 테스트는 순수 판정(`RevertKey.matches`·`AutoConvertPolicy.allowed`, `Tests/ComposerTests.swift`)까지만 본다. 실제 키 이벤트의 modifier 플래그, 설정 앱 → IME defaults 도메인 전달, `client.bundleIdentifier()`가 돌려주는 값은 사람이 확인한다. **구현 시점(2026-09-21) 실기기 미검증.**

- [ ] 설정 → 「영타 변환」 탭의 "되돌리기 키" 팝업에 Shift+Space(기본) · Option+Space · Control+Shift+Space · Option+Shift+Space **네 가지만** 보이고, Control+Space는 없다.
- [ ] 설정을 만진 적 없는 상태(`defaults read com.hyunjincho.inputmethod.haneul haneul.revertKey`가 없음)에서 Shift+Space 되돌리기가 종전과 똑같이 동작한다(회귀 없음).
- [ ] Option+Space로 바꾼 뒤 설정 창을 닫지 않아도 **다음 변환부터** Option+Space가 되돌리고, Shift+Space는 그냥 스페이스가 입력된다.
- [ ] Control+Shift+Space · Option+Shift+Space도 각각 같은 방식으로 되돌린다. 한글 모드(CapsLock 플래그가 붙는 상태)와 ABC 모드 양쪽에서 확인한다.
- [ ] 되돌릴 변환이 없을 때(한글만 친 뒤, 또는 되돌린 뒤 다른 글자를 친 뒤) 고른 조합을 누르면 종전처럼 앱에 그대로 전달된다 — IME가 키를 삼키지 않는다. (Option+Space는 앱에 따라 줄바꿈 없는 공백이 들어갈 수 있음 — 그 앱의 정상 동작.)
- [ ] `defaults write com.hyunjincho.inputmethod.haneul haneul.revertKey garbage` 뒤에도 Shift+Space(기본값)로 동작하고, 설정 창을 열면 팝업이 Shift+Space를 가리킨다.
- [ ] 앱별 끄기: "실행 중인 앱에서 추가..."에 Dock에 보이는 앱만 **이름순**으로 나오고, 메뉴바 전용 앱(HaneulKeyboard 자신 등)·백그라운드 프로세스는 안 나온다. 이미 추가한 앱은 "추가됨"으로 보인다.
- [ ] TextEdit을 목록에 넣은 뒤 TextEdit에서 `apple`을 치면 `메ㅔㅣㄷ`로 남고, 그 상태로 다른 앱(예: 메모)에서는 `apple`로 변환된다 — 설정 창을 닫지 않아도 즉시 반영된다.
- [ ] 목록에서 TextEdit을 지우면 TextEdit에서 다시 변환된다.
- [ ] 위 "영타 자동 변환" 토글을 끄면 목록과 무관하게 모든 앱에서 변환하지 않고, 다시 켜면 목록에 없는 앱만 변환한다.
- [ ] 목록에 있는 앱을 종료한 뒤에도 목록에 이름(또는 bundle ID)이 남아 있고 삭제할 수 있다.
- [ ] 터미널(Terminal.app 또는 Ghostty)을 목록에 넣으면 그 터미널에서 영타 변환이 일어나지 않는다(#30의 "터미널에서 변환 자체가 싫은" 경우의 우회).
- [ ] "전체 제거" 뒤 `defaults read com.hyunjincho.inputmethod.haneul`에 `haneul.revertKey`·`haneul.disabledAppBundleIDs`가 남지 않는다(`haneul.*` 일괄 삭제 경로).
- [ ] (#60으로 자리 이동) 설정 창 **「영타 변환」 탭**에 「되돌리기」·「앱별 자동 변환 끄기」 두 절이 스크롤 없이 보이고, 팝업·추가/제거 버튼·시트가 키보드와 VoiceOver로 조작된다.

### 모든 단어 되돌리기 — 수동 한↔영 토글 (#15, 2026-09-21)

되돌리기 키가 이제 **커서 앞 단어 전부**에 듣는다(자동 변환 여부와 무관). 자동 테스트는 순수 함수(`ManualToggle`·`KoreanComposer.wordBeforeCursor`, 완성형 11,172자 왕복 포함)까지만 본다. **실제 키 이벤트·`selectedRange`/`attributedSubstring` 왕복·앱별 범위 교체 동작은 사람이 확인한다. 구현 시점(2026-09-21) 실기기 미검증.** 설정 키: `defaults read com.hyunjincho.inputmethod.haneul haneul.manualToggleAllWords` (없으면 기본 켜짐).

- [ ] TextEdit 한글 모드에서 `work` + 스페이스 → `재가 `(우리말샘 veto로 변환 안 됨). 여기서 되돌리기 키를 누르면 `work `가 된다.
- [ ] 이어서 한 번 더 누르면 `재가 `로 돌아오고, 계속 눌러도 두 상태를 왔다갔다 한다.
- [ ] `m5` + 스페이스 → `ㅡ5 `. 되돌리기 키 → `m5 `(숫자 `5`는 제자리에 그대로). 다시 누르면 `ㅡ5 `.
- [ ] `k3` → `ㅏ3` 도 같은 방식으로 양방향.
- [ ] 스페이스 없이 `work`까지만 친 상태(조합 중, 화면에 `재가`)에서 되돌리기 키를 누르면 **그 한 번은 확정 + 스페이스**로 끝나고, 그다음 누름부터 `work`로 토글된다.
- [ ] **되돌리기 우선**: `apple` + 스페이스 → `apple `(자동 변환됨). 되돌리기 키 → `메ㅔㅣㄷ `(종전 동작 그대로, 수동 경로로 새지 않는다).
- [ ] 자동 변환을 되돌린 뒤 다시 누르면 `apple `로 돌아온다(종전 동작 그대로).
- [ ] 정상 한국어 단어도 대상이다 — `안녕 ` 뒤에 되돌리기 키를 누르면 `dkssud `가 되고, 다시 누르면 `안녕 `으로 돌아온다(의도된 동작).
- [ ] 문장 중간에서도 **마지막 단어만** 바뀐다: `go work ` → `go 재가 `(앞의 `go`는 그대로).
- [ ] 드래그로 글자를 선택한 상태에서 되돌리기 키를 누르면 토글하지 않는다(선택을 지우고 스페이스가 들어가는 앱의 기본 동작).
- [ ] 빈 입력 칸(커서가 맨 앞)에서 누르면 **스페이스가 그냥 입력된다** — 키가 삼켜지지 않는다.
- [ ] 이모지·한자·한글+영어 혼합(`재work`) 뒤에서 누르면 아무것도 바뀌지 않고 스페이스가 입력된다.
- [ ] 설정 → 「영타 변환」 탭 → `바뀌지 않은 단어도 되돌리기`를 **끄면** `재가 ` 뒤에서 되돌리기 키를 눌러도 바뀌지 않고(스페이스만 입력), `apple` 자동 변환 되돌리기는 여전히 동작한다. 설정 창을 닫지 않아도 즉시 반영된다.
- [ ] 터미널(Terminal.app·Ghostty)에서 `재가 ` 뒤에 되돌리기 키를 눌러도 텍스트가 훼손되지 않는다(#30과 같은 구조적 제약 — 변화 없음이 정상).
- [ ] 되돌리기 키를 Option+Space 등으로 바꿔도(#54) 수동 토글이 그 키로 동작한다.
- [ ] 수동 토글은 **"최근 되돌린 변환"(#53)에 기록되지 않는다** — `재가`↔`work`를 여러 번 토글한 뒤 설정 → 「개인 사전」 탭 → `최근 되돌림` 목록과 `defaults read com.hyunjincho.inputmethod.haneul haneul.recentReverts`에 그 쌍이 없다(자동 변환을 되돌린 것만 쌓인다).
- [ ] 수동 토글로 영어가 된 직후 다음 단어의 자동 변환이 영어 문맥을 받는다: `work ` 상태에서 `로` 계열 짧은 단어(`새` 등)를 치면 화면과 같은 판정이 나온다(오변환이 새로 생기지 않는지 관찰).
- [ ] 시스템 설정 등 네이티브 비밀번호 칸에서는 되돌리기 키를 눌러도 아무 일도 일어나지 않는다(secure input 가드).
- [ ] "전체 제거" 뒤 `defaults read com.hyunjincho.inputmethod.haneul`에 `haneul.manualToggleAllWords`가 남지 않는다.
- [ ] (#60으로 자리 이동) 「영타 변환」 탭 「되돌리기」 절 안에서 `바뀌지 않은 단어도 되돌리기` 토글이 되돌리기 키 팝업 **바로 아래**에 보이고, 키보드·VoiceOver로 조작된다(절이 따로 있지 않다 — 흡수됐다).

### 영타 축약형 — 아포스트로피 (#34, 2026-09-19)

조합 중의 `'`는 이제 경계가 아니라 **단어 내부 문자**다(marked text에 그대로 보인다). 자동 테스트는 composer 층까지만 검증하므로, 실제 앱에서 `'` 키가 IME까지 도달하는지·마크드 텍스트가 어떻게 그려지는지는 사람이 확인한다. 앱마다 스마트 따옴표(`'`→`’`) 치환이 다르므로 **최소 두 종류의 앱**에서 본다.

- [ ] TextEdit에서 한글 모드로 `i'm don't` 를 치면 `i'm don't` 로 변환된다.
- [ ] Chrome 주소창이 아닌 웹 입력창(예: 검색창·댓글창)에서도 같은 결과가 나온다.
- [ ] 치는 동안 `'`가 조합 중 글자(marked text) 안에 보인다 — `ㅑ'` → `ㅑ'ㅡ`.
- [ ] `i'm` 확정 직후 Shift+Space를 누르면 `ㅑ'ㅡ` 로 통째로 되돌려지고, 다시 누르면 `i'm` 으로 돌아온다.
- [ ] `it's a test` 처럼 축약형 뒤 단어도 영어 문맥으로 이어져 변환된다.
- [ ] 한글 입력 중 `안녕'하세요` 를 치면 그대로 `안녕'하세요` 로 남는다(오변환 없음).
- [ ] 여는 따옴표로 시작하는 `'안녕` 이 그대로 입력된다.
- [ ] `'` 를 친 뒤 Backspace를 누르면 `'` 만 지워지고 앞 글자는 남아 조합이 이어진다.
- [ ] `ㅑ'` 까지 친 상태에서 다른 곳을 클릭하면 화면에 보이던 `ㅑ'` 가 그대로 확정된다.
- [ ] 설정에서 영타 자동 변환을 끄면 `i'm` 이 `ㅑ'ㅡ` 로 남는다.

### 개인 사전 (#53, 2026-09-21)

자동 테스트는 판정(`PersonalDictionary.decision`)·composer 합성·되돌리기 기록의 순수 함수까지만 검증한다. **설정 앱 ↔ IME 두 프로세스 사이의 defaults 전달, Shift+Space 시점의 실제 기록, 설정 화면의 렌더링은 자동으로 못 보므로 여기서 사람이 확인한다.** 확인용 명령: `defaults read com.hyunjincho.inputmethod.haneul` (세 키 `haneul.personalDict.force` · `haneul.personalDict.block` · `haneul.recentReverts`).

- [ ] (#60으로 자리 이동) 「개인 사전」 탭의 세그먼트 `변환 추가 | 변환 금지 | 최근 되돌림`으로 세 목록을 각각 볼 수 있고(한 번에 하나), 목록 높이는 셋 다 같다.
- [ ] **변환 추가**: `vismo`를 추가한 뒤 TextEdit 한글 모드에서 `vismo`(퍄느ㅐ) + 스페이스 → **재시작 없이** `vismo`로 변환된다. 목록에서 −로 지우면 다시 `퍄느ㅐ`로 남는다.
- [ ] **변환 추가 — veto 우선**: `cor`을 추가하면 우리말샘 표제어 `책`(cor)도 `cor`로 변환된다. 지우면 다시 `책`.
- [ ] **변환 추가 — 입력 검증**: `Apple`은 `apple`로 저장되고, `app le` · `메ㅔㅣㄷ` · `abc1` · `'`는 빨간 안내 문구와 함께 거부된다. 같은 단어를 두 번 추가해도 한 줄만 있다.
- [ ] **변환 금지(영어)**: `apple`을 금지하면 `apple`(메ㅔㅣㄷ) + 스페이스가 `메ㅔㅣㄷ`로 남고, `apple's`도 `메ㅔㅣㄷ'ㄴ`로 남는다.
- [ ] **변환 금지(한글 표기)**: `메ㅔㅣㄷ`으로 적어도 같은 결과. 영한 혼합(`apple메`)·공백은 거부된다.
- [ ] **양쪽 등록**: 같은 단어를 추가·금지 둘 다에 넣으면 변환되지 않는다(금지 우선).
- [ ] **최근 되돌린 변환 — 기록**: `apple` 변환 직후 Shift+Space로 `메ㅔㅣㄷ`로 되돌리면, 설정 창으로 돌아왔을 때(창을 다시 활성화) 목록 맨 위에 `메ㅔㅣㄷ → apple`이 보인다. `defaults read`에도 같은 쌍이 있다.
- [ ] **기록 방향**: 되돌린 뒤 다시 Shift+Space로 `apple`로 재토글해도 새 항목이 늘지 않는다(영어→한글 방향만 기록). 같은 단어를 다시 되돌리면 중복 없이 맨 위로 올라온다.
- [ ] **기록에 없는 것**: `defaults read` 출력에 되돌린 단어의 한글 표기·영어 외에 문장·앱 이름·시각 같은 값이 **없다.** 되돌리지 않은 일반 한글 입력은 어떤 키에도 남지 않는다.
- [ ] **보안 입력**: 시스템 설정 등 네이티브 비밀번호 칸에서는 변환·되돌리기 자체가 일어나지 않으므로 기록도 생기지 않는다.
- [ ] **"금지" 버튼**: 최근 목록의 항목에서 "금지"를 누르면 그 영어 단어가 변환 금지 목록에 들어가고 최근 목록에서는 사라지며, 이후 그 단어는 변환되지 않는다.
- [ ] **최근 기록 지우기**: 누르면 목록이 비고 `defaults read`에서 `haneul.recentReverts` 키가 사라진다.
- [ ] **자동 변환 끔**: "영타 자동 변환"을 끄면 변환 추가 목록의 단어도 변환되지 않는다(마스터 스위치 우선).
- [ ] **전체 제거**: 세 목록을 채운 뒤 설정 → 「고급」 탭 → 전체 제거 → `defaults read com.hyunjincho.inputmethod.haneul`이 도메인 없음/`haneul.*` 없음이다.
- [ ] **키보드·VoiceOver**: 세 세그먼트의 텍스트 필드·추가·−·금지·지우기 버튼에 Tab으로 도달하고 VoiceOver가 "vismo 삭제" · "apple 변환 금지"처럼 읽는다.

### 단어 제안 (#55, 2026-09-21)

자동 테스트는 **URL 생성 순수 함수**(`WordSuggestion.issueURL` — 제목·본문·인코딩·8000자 상한)까지만 검증한다. **브라우저가 실제로 열리는지, GitHub이 제목·본문·라벨을 그대로 채워 보여주는지는 자동으로 못 보므로 여기서 사람이 확인한다.** ⚠️ 아래는 **전부 미검증**이다 — 구현 세션에서 실기기 확인을 하지 못했다(브라우저를 열지 않기로 한 제약).

> 🔒 이 기능은 **아무것도 전송하지 않는다.** 확인할 것은 "열리는가"이지 "보내지는가"가 아니다. 등록은 열린 GitHub 화면에서 사람이 직접 한다. 확인하다 실제로 이슈를 만들었다면 닫거나 지울 것.

- [ ] (#60으로 자리 이동) 「개인 사전」 탭 아래쪽 「단어 제안」 절의 `단어 제안하기...` 버튼으로 **시트**가 열린다(설정 화면에 폼이 펼쳐져 있지 않다).
- [ ] **빈 입력**: 친 글자·기대 결과가 비어 있으면 시트의 `GitHub에서 열기` 버튼이 **비활성**이다. 둘 중 하나만 채워도 비활성이고, 공백·줄바꿈만 넣어도 비활성이다. 둘 다 채우면 활성된다.
- [ ] **열림**: `메ㅔㅣㄷ` / `apple`을 적고 버튼을 누르면 **기본 브라우저**에 GitHub 이슈 작성 화면이 열린다. (GitHub에 로그인돼 있지 않으면 로그인 화면을 거쳐 같은 화면으로 돌아온다.)
- [ ] **바로 작성 화면이 열린다**(템플릿 고르기 화면이 아님) — 2026-09-21 슈퍼바이저 결정: 이슈 템플릿은 **넣지 않았다**. 템플릿이 있는 저장소에서는 GitHub이 `/issues/new?title=…`을 고르기 화면으로 보내 미리 채운 내용이 날아갈 수 있어서(curl은 로그인 리다이렉트만 돌아와 판별 불가). 나중에 템플릿을 추가하려면 먼저 브라우저로 이 항목을 다시 확인한다.
- [ ] **채워짐**: 열린 화면의 제목이 `[단어 제안] 메ㅔㅣㄷ → apple`이고, 본문에 친 글자·기대 결과·메모·하늘키보드 버전·macOS 버전·확인 체크박스 두 줄이 **마크다운 줄바꿈이 살아 있는 상태로** 들어가 있다. 라벨은 `enhancement`.
- [ ] **종류 전환**: 종류를 "변환 금지 제안"으로 바꾸면 두 번째 칸의 이름이 "바뀐 결과 (원치 않음)"으로 바뀌고, 제목 끝에 `(변환 금지)`가 붙는다.
- [ ] **한글·특수문자**: 메모에 `a+b & c#d 한글 "따옴표"`처럼 적고 열었을 때 브라우저에 **그대로** 보인다 — 특히 `+`가 **공백으로 바뀌지 않는다**(GitHub은 쿼리의 날것 `+`를 공백으로 읽으므로 이게 회귀 지점이다). 줄바꿈이 든 메모도 줄이 유지된다.
- [ ] **아주 긴 메모**: 수천 자를 붙여넣고 열면 브라우저가 URL을 거부하지 않고 열리며, 본문 끝에 `…(길이 제한으로 잘림)`이 보인다.
- [ ] **입력란 비우기**: 버튼을 눌러 창이 열린 뒤 설정 화면의 세 입력란이 비어 있다.
- [ ] **최근 되돌린 변환 → 제안**: `apple`을 Shift+Space로 되돌린 뒤 설정의 "최근 되돌린 변환" 행에서 **"제안"**을 누르면, 제목이 `[단어 제안] 메ㅔㅣㄷ → apple (변환 금지)`로 채워진 화면이 열린다. **"금지" 버튼과 헷갈리지 않게 둘 다 보이고**, "제안"을 눌러도 그 항목이 목록에서 사라지지 않는다.
- [ ] **전송 없음**: 위 동작 중 앱이 네트워크를 쓰지 않는다 — 열린 브라우저 탭을 그냥 닫으면 GitHub에 아무것도 남지 않는다.
- [ ] **키보드·VoiceOver**: 세 입력란·종류 Picker·버튼에 Tab으로 도달하고, VoiceOver가 최근 목록의 두 버튼을 "apple 변환 금지" / "apple 변환 금지 제안하기"로 구분해 읽는다.

## 키보드 접근성·VoiceOver

- [ ] 키보드만 사용해 메뉴 항목과 설정 창의 모든 제어요소로 포커스를 이동하고 실행할 수 있다.
- [ ] 포커스 표시가 보이고, Tab/Shift+Tab 순서가 화면의 논리적 순서와 일치한다.
- [ ] VoiceOver를 켠 뒤 메뉴바 아이콘이 `HaneulKeyboard`로 읽힌다.
- [ ] VoiceOver가 메뉴 항목, 설정 제어, 온보딩 제목·버튼의 이름과 현재 상태를 이해 가능하게 읽는다.
- [ ] VoiceOver가 켜진 상태에서 입력 소스 전환, 설정 열기, 온보딩 진행이 정상 동작한다.
- [ ] (#60) 탭바에 포커스를 둔 상태에서 `←`/`→`로 탭 5개를 모두 이동할 수 있고, `Tab`만으로 각 탭의 모든 제어요소에 닿는다.
- [ ] (#60) 개인 사전 세그먼트에 포커스를 두고 `←`/`→`로 세 칸을 이동할 수 있다(VoiceOver가 "개인 사전 목록 선택"으로 읽는다).
- [ ] (#60) 시트 2종에서 `Esc`가 닫기/취소로 동작하고, 단어 제안 시트에서 `Return`이 `GitHub에서 열기`로 동작한다.
- [ ] (#60) VoiceOver가 탭 5개 이름, 상태 배지(심볼+문구 한 덩어리), `⊖`·`금지`·`제안` 버튼, 개인 사전 입력칸("변환 추가할 영어 단어" 등)을 이해 가능하게 읽는다.

## 설치 도메인과 전체 제거 (review0706 P2-2)

- [ ] `/Library/Input Methods`에 system-domain 설치본이 있는 상태에서 앱의 "IME 설치" 버튼을 누르면, 새로 복사하지 않고 기존 시스템 설치본을 재등록/활성화한다.
- [ ] system-domain 설치본과 `~/Library/Input Methods`의 user-domain 잔재가 동시에 존재하는 상태에서 "IME 설치"를 실행하면 user-domain 잔재가 정리되고 system-domain만 남는다.
- [ ] `/Applications`에 이름이 다른 HaneulKeyboard 복사본(예: 파일명을 바꾼 앱)이 있는 상태에서 "전체 제거"를 실행하면, 두 복사본 모두 파일이 삭제되고 macOS 재부팅 후 Launchpad/Spotlight에 중복 항목이 남지 않는다.

## 실패 시나리오 (review-0712 P3-8)

자동 테스트는 "무엇을 결정하는가"까지만 검증한다(`Tests/ComposerTests.swift`의 설치·제거 결정 로직 항목 — `AppMoveDecision`·`UninstallDecision`·`UninstallOutcome`). 실제 관리자 프롬프트·파일 삭제·GUI 경고는 아래에서 사람이 확인한다.

### 전체 제거 — 관리자 암호 취소

- [ ] "전체 제거" 실행 중 관리자 암호 창에서 **취소**를 누르면 결과 창이 `IME 번들 삭제: 실패` / `메인 앱 삭제: 건너뜀(재시도용 보존)`으로 표시되고, "완료"라고 말하지 않는다.
- [ ] 위 상태에서 "확인"을 눌러도 **앱이 종료되지 않고** 남아, 같은 화면에서 "전체 제거"를 곧바로 다시 실행할 수 있다.
- [ ] 재실행에서 암호를 올바르게 입력하면 IME·메인 앱이 모두 삭제되고 "완료" 문구가 나온다.

### 앱 이동 — 관계없는 목적지 앱

- [ ] `/Applications`에 **관계없는 앱**을 `HaneulKeyboard.app`이라는 이름으로 둔 상태에서(다른 앱을 복사·개명), 다운로드 폴더의 HaneulKeyboard를 실행해 "옮기고 다시 열기"를 누르면 → "응용 프로그램 폴더에 다른 앱이 있어요" 경고가 뜨고 **그 앱이 덮어써지지 않는다**.
- [ ] 앱 파일명을 바꿔서(예: `내키보드.app`) 실행한 뒤 이동을 진행하면 `/Applications/HaneulKeyboard.app`으로 들어간다(바꾼 이름 그대로 들어가지 않는다).

### 온보딩 — 완료 시 창 닫힘

- [ ] `defaults delete com.hyunjincho.haneulkeyboard haneul.hasCompletedOnboarding` 후 앱을 실행해, 온보딩 마지막 단계의 **"완료"를 누르면 창이 즉시 닫힌다**(빨간 닫기 버튼을 쓰지 않아도 된다).

### 시스템 설치 스크립트 — 중단

- [ ] `scripts/build_notarize_install.sh` 실행 중 staging 검증 단계에서 실패했을 때(예: 일부러 서명을 깨뜨려서), `/Library/Input Methods`의 **기존 설치본이 그대로 남아 있다**(설치 전 상태 보존).

## 앱 삭제 ↔ IME 연동 (#32, 2026-09-19)

자동 테스트는 판단 함수(`OrphanDecision`)만 검증한다. 실제 삭제·비활성화·종료는 아래에서 사람이 확인한다. 로그: `/usr/bin/log show --predicate 'subsystem == "com.hyunjincho.haneulkeyboard" AND category == "orphan"' --last 30m`.

> ⚠️ **전제 조건 (2026-09-20, #43 · 리뷰 F-4)** — 개발 머신에서는 이 절이 **그냥은 통과할 수 없다.** IME는 후보 경로를 LaunchServices(`urlsForApplications`)에서도 가져오는데, `.build/*DerivedData*/Build/Products/*/HaneulKeyboard.app` 같은 빌드 산출물이 등록돼 있고 디스크에 실존하면 휴지통 밖에 살아 있는 복사본으로 세어 **항상 `present`** 가 된다(2026-09-20 Mac Studio 실측: 메인 앱 bundle ID로 10개 경로, 그중 9개가 `.build` 산출물). 그러면 앱을 휴지통에 버려도 자기 정리는 절대 발동하지 않고 "연동이 안 된다"는 오판이 난다. 검증 전에:
> 1. 저장소 안 `.build/*DerivedData*` 산출물을 지우거나(`rm -rf`는 오너 결정) `lsregister -u <산출물 경로>`로 등록만 뺀다. (2026-09-21 #52: `scripts/build_notarize_install.sh`는 끝날 때 자기 산출물 `.build/DerivedData/Build/Products/Release/`의 등록을 자동으로 뺀다 — 다른 DerivedData·Debug 산출물은 여전히 손으로.)
> 2. 확인 — 아래 프로브가 `/Applications/HaneulKeyboard.app` **하나만** 돌려줘야 한다 (프로브가 확인 — scripts/postinstall_probe.sh 4번):
>    `swift -e 'import AppKit; NSWorkspace.shared.urlsForApplications(withBundleIdentifier: "com.hyunjincho.haneulkeyboard").forEach { print($0.path) }'`
> 3. 최종 사용자 머신에는 DerivedData가 없으므로 이 전제는 개발 머신 전용이다(제품 결함 아님).

- [ ] `/Applications/HaneulKeyboard.app`을 휴지통에 버린 뒤(메뉴바 앱 종료) 한글 모드로 계속 쓰면, **2분 이상 지나** 입력이 ABC로 넘어가고 `~/Library/Input Methods/HaneulKeyboardIM.app`이 사라진다. 로그에 `in the Trash — waiting` → `on repeated observations` → `removed own bundle`이 남는다.
- [ ] 위 상태에서 로그의 `TISDisableInputSource=true/false`를 기록한다 — 프로그램 비활성화가 IME 프로세스 컨텍스트에서 되는지가 여기서 처음 확정된다. false면 입력 소스 목록의 "하늘키보드" 항목은 재로그인 후 사라지는 것이 정상.
- [ ] **복원 유예**: 앱을 휴지통에 버렸다가 **2분 안에 "제자리에 놓기"**로 되돌리면 IME가 사라지지 않고 설정도 남아 있다.
- [ ] **오탐 없음**: 앱을 새 버전으로 교체(Finder "대치")한 직후 한글 모드로 전환해도 IME가 사라지지 않는다(옛 번들이 휴지통에 있어도 `/Applications`에 새 번들이 있으면 present).
- [ ] **오탐 없음**: 앱을 `/Applications/Utilities/` 같은 하위 폴더나 `~/Desktop`으로 **옮기기만 하고 실행하지 않은** 상태에서 한글 모드로 30분 이상 써도 IME가 사라지지 않는다(휴지통에 옛 버전 사본이 있어도 inode가 달라 무시 — 기록 경로의 사본이든 LaunchServices가 기억하는 사본이든, #47).
- [ ] **(#47 확인, 2026-09-20)** 옛 버전 `HaneulKeyboard.app` 사본을 휴지통에 둔 상태에서 위 전제 조건의 프로브(`urlsForApplications`)를 돌려 `~/.Trash` 경로가 **포함되는지** 기록한다 — 포함되면 LaunchServices 후보에 대한 inode 필터가 실제로 작동하는 경로이고, 그 상태에서 앱을 `~/Desktop`으로 옮겨 두면 로그가 `not found — waiting`(30분 유예)이어야 한다(`in the Trash`가 아님).
- [ ] **오탐 없음**: 앱이 외장 볼륨에 있고 볼륨을 **마운트하지 않은** 세션에서 한글 모드를 30분 미만으로 쓰면 아무 일도 없다(30분 이상이면 정리되는 것이 현재 설계 — 재설치로 복구).
- [ ] **ABC 없음**: 시스템 설정 입력 소스에서 ABC를 빼 둔 상태로 앱을 휴지통에 버리면, IME는 정리를 **보류**하고(로그 `could not switch … deferred`) 계속 쓸 수 있다. 입력 소스를 잃는 상태가 생기지 않는다.
- [ ] 휴지통을 비운 뒤(앱이 어디에도 없음) IME는 **30분 이상** 지나 두 번째 관찰에서 정리된다 — 첫 관찰에서 바로 사라지지 않는다.
- [ ] 역방향: 시스템 설정 → 입력 소스에서 "하늘키보드"를 "-"로 뺀 뒤 메뉴바 메뉴를 열면 "한글 입력기가 꺼져 있음 — 다시 켜기"가 보이고, 누르면 입력 소스 목록에 다시 나타난다.
- [ ] 전체 제거: "전체 제거"를 실행하면 결과 창에서 "시스템 설정에서 직접 빼세요" 안내가 **더 이상 나오지 않는지**(프로그램 비활성화 성공 시) 또는 종전처럼 나오는지 기록한다.
- [ ] 설정 → 「일반」 탭 → "입력기 제거" (#46, 2026-09-20 · 문구는 #60에서 "IME 제거" → "입력기 제거"): 실행 뒤 시스템 설정 입력 소스 목록에서 "하늘키보드"가 **사라지는지**(프로그램 비활성화 성공) 또는 종전처럼 남는지 기록한다 — 전체 제거·자기 정리와 같은 `TISDisableInputSource` 경로. 로그 `IME 제거 — TISDisableInputSource=true/false`(subsystem `com.hyunjincho.haneulkeyboard`, category `main`).


## 자동 업데이트 (#19, 2026-09-21)

자동 테스트(`Tests/ComposerTests.swift`의 「자동 업데이트 판단」 묶음)는 **판단만** 검증한다 — CalVer 비교, 자산 선택, 허용 호스트, 24시간 throttle, 검증 5종 합성, 교체 전략. **네트워크 요청·다운로드·서명 검증 실행·앱 교체·재실행·IME 갱신은 자동으로 확인할 방법이 없어 전부 아래에서 사람이 본다.** 2026-09-21 구현 시점에 이 절은 **한 줄도 실기기 검증되지 않았다.**

> **준비 — 시험용 릴리스**: 진짜 릴리스를 올리지 않고 끝까지 보려면 Debug 빌드의 시험 훅을 쓴다.
> `defaults write com.hyunjincho.haneulkeyboard haneul.updateRepoOverride Hyunjin-Cho/HaneulKeyboard-updatetest`
> (Release 빌드에는 이 키를 읽는 코드가 **컴파일되지 않는다** — Debug 전용.) 시험 저장소에는 현재 앱보다 높은
> tag(예: `2026.99`)와 **같은 이름 규칙의 자산** `HaneulKeyboard_2026.99.zip`(서명·공증까지 끝낸 진짜 빌드)을 올려 둔다.
> 끝나면 `defaults delete com.hyunjincho.haneulkeyboard haneul.updateRepoOverride`.
> 요청이 실제로 나가는지 / 안 나가는지는 `/usr/bin/nettop -p <pid>` 또는 Charles·Proxyman 같은 도구로 본다.
> 로그: `/usr/bin/log show --predicate 'subsystem == "com.hyunjincho.haneulkeyboard"' --last 30m | grep -i update`

### 확인 (네트워크 요청)

- [ ] 설정 창 「업데이트」 탭에 업데이트 절이 있고, "현재 버전"이 실제 설치본의 `버전 (빌드)`와 일치한다.
- [ ] **"지금 확인"** → 잠시 "확인 중…" 후 결과가 나온다. 최신이면 "최신 버전입니다.", 새 버전이 있으면 "새 버전 \<tag\>이 있어요."와 **[업데이트]** 버튼, "릴리스 노트 보기" 링크가 나타난다.
- [ ] 확인에 성공하면 **"마지막 확인"** 시각이 지금으로 갱신된다.
- [ ] 새 버전이 있을 때 **메뉴바 메뉴**에도 "새 버전 \<tag\> 사용 가능 — 업데이트..." 항목이 뜨고, 누르면 설정 창이 열린다.
- [ ] 네트워크를 끊고 "지금 확인" → 빨간 "실패: 네트워크 오류 — …" 문구가 뜨고 **앱이 멈추거나 죽지 않는다**. "마지막 확인" 시각은 **갱신되지 않는다**(실패는 기록하지 않는다).

### 토글 OFF — 요청 0 (프라이버시 주장의 근거)

- [ ] **"업데이트 자동 확인"을 끈** 뒤 앱을 완전히 종료하고 다시 실행한다 → 실행 후 **최소 2분** 동안 api.github.com으로 나가는 요청이 **하나도 없다**(위 nettop/프록시로 확인). 로그에도 `update check` 줄이 없다.
- [ ] 그 상태에서 **"지금 확인"을 누르면** 요청이 **그때 딱 한 번** 나간다.
- [ ] 토글 상태가 앱을 재실행해도 유지된다(`defaults read com.hyunjincho.haneulkeyboard haneul.updateAutoCheck` → `0`).

### 자동 확인 · 24시간 throttle

- [ ] 토글 **ON** 상태로 앱을 실행하면 **약 15초 뒤** 확인 요청이 **한 번** 나간다(즉시가 아니다 — 앱 이동·메뉴바 생성이 먼저).
- [ ] 곧바로 앱을 다시 실행하면 **요청이 나가지 않는다**(24시간 throttle). "마지막 확인" 시각도 그대로다.
- [ ] `defaults delete com.hyunjincho.haneulkeyboard haneul.updateLastCheck` 후 재실행하면 다시 요청이 나간다.
- [ ] 새 버전이 있어도 **자동으로 내려받거나 설치하지 않는다** — 알림만 뜨고 `/Applications`의 앱이 그대로다.

### 검증 실패 — 기존 앱이 그대로 남는가 (가장 중요)

> 각 항목 뒤에 **`/Applications/HaneulKeyboard.app`의 빌드 번호가 그대로인지** 반드시 확인한다.

- [ ] **서명 깨뜨리기**: 시험 저장소 자산을 *서명하지 않은* 또는 *ad-hoc 서명한* 빌드로 바꾼 뒤 [업데이트] → "검증 실패로 설치하지 않았어요: …" 가 뜨고 **기존 앱이 그대로**다.
- [ ] **공증 없음**: 서명은 했지만 **노타리·staple 하지 않은** zip → `Gatekeeper(노타리) 거부`로 실패하고 기존 앱이 그대로다.
- [ ] **다른 앱**: 자산을 전혀 다른 앱(예: 계산기)의 zip으로 바꾸면 `번들 ID가 HaneulKeyboard가 아님`으로 실패한다.
- [ ] **다운그레이드**: 시험 저장소 tag만 높이고 자산은 **현재보다 낮은 빌드**의 zip으로 두면 `버전이 현재보다 높지 않음`으로 실패한다.
- [ ] **파일 손상**: 자산 zip의 바이트 일부를 훼손하면 "압축 해제 실패" 또는 크기 불일치로 멈추고 기존 앱이 그대로다.
- [ ] 위 실패 뒤에도 **메뉴바 앱이 계속 살아 있고** 설정 창에서 다시 "지금 확인"을 할 수 있다.

> ⚠️ **2026-09-21 (#19 보안 검토)**: "서명 Team이 현재 앱과 다름" 항목은 **Debug 빌드에서 검증되지 않는다** — Debug는 `IMEInstaller.isSameTeamSignedBundle`이 무조건 `true`를 돌려준다(미서명 개발 빌드를 위한 설계). **Release 빌드로 따로 확인할 것.**
> 또 **Debug + `haneul.updateRepoOverride`** 조합은 임의 저장소의 공증된 앱이면 설치되는 **개발 머신 한정 함정**이다(Release 빌드에는 이 키를 읽는 코드가 컴파일되지 않는다). 시험이 끝나면 반드시 `defaults delete`.

### 교체 · 재실행

- [ ] 정상 자산으로 [업데이트] → "다운로드 중…" → "서명·노타리 검증 중…" → "설치 중…" 순으로 문구가 바뀐 뒤 앱이 **스스로 다시 열린다**(메뉴바 아이콘이 잠깐 사라졌다 돌아온다).
- [ ] 재실행된 앱의 설정 → 「업데이트」 탭 "현재 버전"이 **새 버전·새 빌드**를 가리킨다.
- [ ] `/Applications` 안에 `.HaneulKeyboard.app.update-…` 나 `.HaneulKeyboard.app.old-…` 같은 **찌꺼기가 남아 있지 않다**.
- [ ] 교체 중 `~/Library/Preferences/com.hyunjincho.*` 설정(개인 사전·되돌리기 키·앱별 끄기)이 **그대로 남아 있다**.
- [ ] **응용 프로그램 폴더 밖**에서 실행한 앱(예: 다운로드 폴더 사본)에서 [업데이트]를 누르면 "자동 업데이트는 응용 프로그램 폴더(/Applications)에 설치된 앱에서만 할 수 있어요"라며 **중단**되고, `/Applications`의 앱은 건드려지지 않는다.
- [ ] **root 소유 설치본**(`sudo chown root /Applications/HaneulKeyboard.app`)에서 [업데이트] → **관리자 암호 창**이 뜬다. **취소**하면 "앱 교체 실패 — 기존 앱은 그대로예요"가 뜨고 실제로 기존 앱이 그대로다. 암호를 넣으면 교체·재실행된다.

### IME 갱신 (재실행 후)

- [ ] 사용자 도메인(`~/Library/Input Methods`) 설치본: 새 버전으로 재실행되면 **자동으로 IME가 갱신**된다 — `HaneulKeyboardIM.app`의 `CFBundleVersion`이 새 빌드와 같아지고, 로그에 `IME refreshed to bundled build …`가 남는다. **한글 입력이 갱신 뒤에도 정상 동작한다**(입력기가 죽은 채로 남지 않는다).
- [ ] 갱신 뒤 시스템 설정 → 입력 소스 목록에 "하늘키보드"가 **중복 없이 하나만** 남아 있고, 사용자 클릭 없이 계속 선택 가능하다. (⚠️ 사용자 클릭 없이 TIS 활성화가 되는지는 이 항목이 **처음 확정**한다 — 안 되면 아래 폴백으로 내려간다.)
- [ ] 시스템 도메인(`/Library/Input Methods`) 설치본: 자동 갱신 대신 「업데이트」 탭에 "한글 입력기 갱신 필요" 경고가 뜨고, 로그에 `system-domain (root) — manual refresh needed`가 남는다.
- [ ] 위 경고 상태에서 안내대로 직접 다시 설치하면(또는 설정의 "IME 설치"), **경고가 그 자리에서 사라진다**(앱을 재실행하지 않아도).
- [ ] IME 빌드가 이미 최신이면 앱 시작 시 **아무 갱신도 일어나지 않는다**(로그에 refresh 줄 없음 — 시작 시 복사 금지 원칙 H-01 유지).
